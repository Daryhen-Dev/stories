...
- [x] **GREEN** Implement `src/server.ts` (`buildServer({ db, makeAdapter })` factory), `src/plugins/loopback-guard.ts` (Host allowlist `127.0.0.1:<port>` / `localhost:<port>` / `[::1]:<port>` + Origin allowlist), `src/plugins/redact.ts`, logger serializers, `src/routes/health.ts`; scanner passes on the real tree (`.env*` git-ignored per bootstrap). Tests pass. Record evidence. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** Host `[::1]:3789` accepted; IPv6-mapped forms rejected; redaction hits nested objects and arrays; port override reflected in the allowlist. <!-- sdd-owner: implementation -->
- [x] **REFACTOR** Collapse guard checks into one `onRequest` hook with typed 403 payloads. <!-- sdd-owner: implementation -->
- [x] **Verify & bounds** Suites green; PM R5 scenarios + PM R2 repo-scan scenario pass; diff ≤400 lines; record evidence. <!-- sdd-owner: implementation -->
...
- [x] **RED** Write `packages/local-api/test/stories.test.ts`: multipart creation sends required scalar metadata (`type`, `expiresAt`, `position`, `mediaSize`) before `media`, carries optional `durationSeconds` and `posterSize` before an optional poster, and passes a guarded stream + declared `contentLength` end-to-end; invalid `expiresAt` or file-first ordering → typed 400 with no upload; declared or actual oversize → typed 413 with no story row; creation inserts status `published` and auto-publishes (manifest bytes change without a second call); `PATCH /api/stories/:id` re-validates the window; `DELETE /api/stories/:id` removes the row + pending-deletion record; poster is optional and stored. All fail. Record evidence. <!-- sdd-owner: implementation -->
- [x] **GREEN** Implement `src/routes/stories.ts`, `src/routes/story-edit.ts`, and a focused multipart metadata parser that consumes the PR 10A guard (not a new buffering implementation): parse metadata → reject file-before-metadata → stream/verify media and optional poster → insert → auto-publish via the PR 6 service. Tests pass. Record evidence. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** Video with `durationSeconds`; adapter upload failure surfaces the typed code and creates no row; publication failure retains local truth and reports its failure separately; declared-versus-actual size mismatch remains typed. <!-- sdd-owner: implementation -->
- [x] **REFACTOR** Keep field-first parsing and typed route error mapping cohesive; do not duplicate the PR 10A stream guard. <!-- sdd-owner: implementation -->
- [x] **Verify & bounds** Suites green; SL R1/R2/R7 + MP R4 creation scenarios pass; diff ≤1,500 lines; record evidence. <!-- sdd-owner: implementation -->
...
- [x] **RED** Write `src/viewer-ux.test.ts` (happy-dom): per-story progress bars; prev/next via tap zones and arrow keys; pause on pointer-hold; photo renders, video renders with `posterUrl` and media-only fallback when absent (SEV R1 scenarios); `src/refresh.test.ts`: re-fetch when the tab becomes visible if last fetch >60 s, no re-fetch under 60 s (SEV R5 scenarios); `src/bundle-size.test.mjs` (runs post-build): `dist/stories-viewer.iife.js` exists and gzipped size ≤50 KB. Runtime behavior tests fail before implementation; the generated-artifact test is deliberately post-build. Record evidence. <!-- sdd-owner: implementation -->
- [x] **GREEN** Implement the viewer template/styles (full-screen, zero external assets), `vite.config.ts` dual build (ESM `dist/index.js` + types; IIFE `dist/stories-viewer.iife.js` with Lit + Zod + styles inlined), `/register` side-effect entry in the npm build. Tests + build pass; size budget met. Record evidence. <!-- sdd-owner: implementation -->
- [x] **TRIANGULATE** Story expiring mid-session skipped on next navigation (UI level); keyboard focus handling; `posterUrl` absent → no error and no broken image; pointer release outside the viewer resumes only on document-level release. <!-- sdd-owner: implementation -->
- [x] **REFACTOR** Deduplicate progress-bar timing/load paths; assert no SSR code path in the bundle (string scan test). <!-- sdd-owner: implementation -->
- [x] **Verify & bounds** Tests + build green; SEV R1/R5/R6 scenarios pass; IIFE ≤50 KB gzipped; native review approved and acknowledged after the deterministic TTL-test correction; maintainer authorized ordinary-repository delivery; evidence recorded. <!-- sdd-owner: implementation -->
...
- [x] **B1 RED/STRUCTURE** Add a Next photo smoke at `/next/` before the app exists; it MUST await the manifest response at the provider origin and assert its short TTL plus the rendered provider-origin photo. It fails with no `stories-viewer` before the demo route exists. Record evidence. <!-- sdd-owner: implementation -->
- [x] **B1 GREEN/TRIANGULATE/REFACTOR** Scaffold static `apps/demo-next` with exact Next/React host pins and client-only `dynamic(() => import('@stories/stories-embed/register').then(...), { ssr: false })` registration; extend fixed-origin static serving/build orchestration and make the Next photo smoke plus existing smokes pass. Record evidence. <!-- sdd-owner: implementation -->
